...
def copiar_al_portapapeles(event):
    ...
    if seleccion:
        item = tree.item(seleccion)  # Obtener los datos de la fila
        valores = item["values"]  # Lista de valores en la fila

        if valores:  # Verificar si hay datos
            columna = tree.identify_column(event.x)  # Identificar columna
            indice_columna = int(columna[1:]) - 1  # Convertir columna a índice
            
            if 0 <= indice_columna < len(valores):  # Asegurar que el índice sea válido
                texto_copiado = str(valores[indice_columna])  # Obtener texto de la celda
                ventana.clipboard_clear()  # Limpiar portapapeles
                ventana.clipboard_append(texto_copiado)  # Copiar texto
                ventana.update()  # Actualizar portapapeles

                # Opcional: Mostrar un mensaje en la barra de estado
                etiqueta_estado.config(text=f"Copiado: {texto_copiado}")
    
# Edición ventana que contiene la base de datos y los botones de edición y búsqueda
def mostrar_ventana_principal():
    global tree, entrada_cod_prod, entrada_descripcion, entrada_observacion, ventana, etiqueta_estado # <-- Asegura que sean variables globales
    ventana = tk.Tk()
    ...
    # Asocia el evento de doble clic para copiar al portapapeles
    ...
    # Etiqueta para mostrar estado ("Copiado: ")
    etiqueta_estado = tk.Label(ventana, text="", bg="royalblue4", fg="snow")
    etiqueta_estado.pack(fill=tk.X, pady=5)
