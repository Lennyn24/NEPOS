...
def buscar_elemento():
    """Realiza la búsqueda en tiempo real con coincidencias parciales."""
    criterio = entrada_busqueda.get().strip().lower()
    if not criterio:
        cargar_datos()  # Si está vacío, recarga todos los datos
        return
    ...
    try:
        ...
        cursor = conn.cursor()
        consulta = """
            SELECT * FROM PRODUCTOSN 
            WHERE LOWER(Descripcion) LIKE ? 
            OR LOWER(Observacion) LIKE ?
        """
